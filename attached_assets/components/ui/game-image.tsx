...
export function GameImage({
  ...
  const handleError = () => {
    ...
    if (retryCount < 3) {
      ...
      if (retryCount === 0) {
        // First try: themed placeholder based on item name
        const theme = getImageTheme(alt)
        setCurrentSrc(`https://source.unsplash.com/150x150/?${theme}`)
        setRetryCount(1)
        return
      } else if (retryCount === 1) {
        // Second try: Picsum with themed seed
        const seed = itemId || alt.replace(/\s+/g, "-").toLowerCase()
        setCurrentSrc(`https://picsum.photos/seed/${seed}/150/150`)
        setRetryCount(2)
        return
      } else if (retryCount === 2) {
        // Third try: Another Picsum with random seed
        setCurrentSrc(`https://picsum.photos/150/150?random=${Math.random().toString(36).substring(7)}`)
        setRetryCount(3)
        return
      }
    }

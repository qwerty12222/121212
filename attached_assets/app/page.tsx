...
export default function GiftsBossGame() {
  ...
  const handleCreateStarsInvoice = async () => {
    ...
    if (typeof window !== "undefined" && window.Telegram?.WebApp) {
      ...
      try {
        // This would normally create a real invoice through Telegram Bot API
        tg.showAlert(`Creating invoice for ${starsAmount} Telegram Stars...`)

        // Simulate successful payment
        setTimeout(() => {
          setBalance((prev) => ({ ...prev, stars: prev.stars + starsAmount }))
          tg.showAlert(`Purchase successful! +${starsAmount} Stars added to your balance.`)
